...
class Filter:
    ...
    def compute_x_post_test(self, state, obs, task_net=None, use_initial_state=True):  # obs:[seq_num, y_dim, seq_len]
        if task_net is None:
            temp_net = self.train_net
        else:
            temp_net = task_net
        self.reset_test(state.shape[0])
        temp_net.initialize_hidden(is_train=False)
        seq_num, y_dim, seq_len = obs.shape
    ...
    def filtering(self, observation, task_net):
        ...
        x_predict, F_jacob = self.model.f_nonlinear(x_last, jacobian=True)
        ...
        y_predict, H_jacob = self.model.hRotate(x_predict, jacobian=True)
        ...
        x_predict_k = self.model.f_nonlinear(x_post)
        y_predict_k = self.model.hRotate(x_predict_k)
    ...
    def ekf_filtering(self, observation, Q, R):
        ...
        # F_jacob = self.model.Jacobian_f(x_last, jacobian=True)
        # H_jacob = self.model.Jacobian_g(x_predict)
    ...
    def reset_test(self, batch_size: int):
        """
        重置滤波器内部状态，用于 test 阶段，batch_size 来自输入数据。
        """
        self.training_first = True
        # state_post、y_predict_history 和 state_history 都按 batch_size 来初始化
        self.state_post = torch.ones((batch_size, self.model.x_dim, 1), device=self.device)
        self.y_predict_history = torch.zeros((batch_size, self.model.y_dim, 1), device=self.device)
        self.state_history = self.state_post.clone()
