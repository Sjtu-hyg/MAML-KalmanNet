...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        if step % 1000 == 0 and step != 0:
            torch.save(maml.base_net.state_dict(), './MAML_model/nonlinear/basenet_' + str(step) + '.pt')

if __name__ == '__main__':

    argparser = argparse.ArgumentParser()
    argparser.add_argument('--epoch', type=int, help='epoch number', default=30001)
